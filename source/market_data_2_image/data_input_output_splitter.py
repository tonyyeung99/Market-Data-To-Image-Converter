...
#  - output: the last Y (e.g. 60) minutes of the trading day
#############################################
class MarketDataSplitter:
    ...
    def split(self):
        if self.extract_obj.md_date.shape[0] > self.threshold:
            ...
            input_extract_obj = extracts.md_extracts(input_date, input_time, input_open, input_close, input_high,
                                                     input_low, input_volume)
            ...
            input_extract_obj = extracts.md_extracts(np.array([]), np.array([]), np.array([]), np.array([]),
                                                     np.array([]),
                                                     np.array([]), np.array([]))
            output_extract_obj = extracts.md_extracts(np.array([]), np.array([]), np.array([]), np.array([]),
                                                      np.array([]),
                                                      np.array([]), np.array([]))

import numpy as np
import source.data.market_data_2_image.data_input_output_splitter as sp_pkg
...
class TestInOutSplitter(unittest.TestCase):
    ...
    def testSplitDataWithinThresold(self):
        splitter = sp_pkg.input_output_splitter(self.extract_obj, 3)
    ...
    def testSplitDataTouchThresold(self):
        splitter = sp_pkg.input_output_splitter(self.extract_obj, 6)
        split_flag, input_split_obj, output_split_obj  = splitter.split()
        self.assertEqual(split_flag, False)


    def testSplitDataOutThresold(self):
        splitter = sp_pkg.input_output_splitter(self.extract_obj, 7)
        split_flag, input_split_obj, output_split_obj = splitter.split()
        self.assertEqual(split_flag, False)
